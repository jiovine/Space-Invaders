...
class Invaders(GameApp):
    ...
    def Update_STATE_INACTIVE(self):
        """ Updates states if state in Inactive """
        if self._state == STATE_INACTIVE:
            self._text = GLabel(text="Press space to start...", x=(GAME_WIDTH/2.0),y=(GAME_HEIGHT/2.0), font_size=60,font_name="Arcade.ttf")
    
    def Update_STATE_NEWWAVE(self):
        """ Updates State if State is new wave """
        if self._state == STATE_NEWWAVE:
            self._wave = Wave()
            self._state = STATE_ACTIVE

    def Update_STATE_ACTIVE(self, dt):
        """ Updates State when state is active """
        if self._state == STATE_ACTIVE:
            self._wave.update(self.input, dt)
            if self._wave.getPause() == 1 and self._wave.getLives()>0:
                self._state = STATE_PAUSED
            if self._wave.getLives()==0 or self._wave.getVictory():
                self._state = STATE_COMPLETE

    def Update_STATE_PAUSED(self):
        if self._state == STATE_PAUSED:
            self._text = GLabel(text="Press space to continue", x=(GAME_WIDTH/2.0),y=(GAME_HEIGHT/2.0), font_size=60,font_name="Arcade.ttf")
            self._text1 = GLabel(text="You have " + str(self._wave._lives) + " lives left" , x=(GAME_WIDTH/2.0),y=(GAME_HEIGHT/2.0)-50 , font_size=40,font_name="Arcade.ttf")
            self._determineState()
    ...
    def Update_STATE_COMPLETE(self):
        if self._state == STATE_COMPLETE:
            if self._wave.getLives() > 0:
                self._text = GLabel(text="Press space to start next wave", x=(GAME_WIDTH/2.0),y=(GAME_HEIGHT/2.0)+30, font_size=45,font_name="Arcade.ttf")
            else:
                self._text = GLabel(text="Press space to start a new game", x=(GAME_WIDTH/2.0),y=(GAME_HEIGHT/2.0)+30, font_size=45,font_name="Arcade.ttf")
            if self._input.is_key_down(('spacebar')):
    ...
    def _determineState(self):
        """ Checks for a keypress and updates state accordingly"""
        change = self.input.is_key_down('spacebar')

        #dismisses welcome screen and stars new wave if 's' was pressed
