...
def tensor_map(fn: Callable[[float], float]) -> Any:
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _map(
        ...
        out_index, in_index = np.array([0 for _ in range((len(out_shape)))]), np.array(
            [0 for _ in range((len(in_shape)))]
        )
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _zip(
        ...
        out_index, a_index, b_index = (
            np.array([0 for _ in range((len(out_shape)))]),
            np.array([0 for _ in range((len(a_shape)))]),
            np.array([0 for _ in range((len(b_shape)))]),
        )
        for i in range(len(out)):
            to_index(i, out_shape, out_index)
            ...
            out[index_to_position(out_index, out_strides)] = fn(
                ...
                b_storage[int(index_to_position(b_index, b_strides))],
            )
    ...
    def _reduce(
        ...
        for i in range(len(out)):
            ...
            for j in range(a_shape[reduce_dim]):
                ...
                out[k] = fn(a_storage[in_pos], out[k])
