...
class CNNSentimentKim(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        layer1 = self.conv1(embeddings).relu()
        layer2 = self.conv2(embeddings).relu()
        layer3 = self.conv3(embeddings).relu()
        embeddings = (minitorch.max(layer1, 2) + minitorch.max(layer2, 2) + minitorch.max(layer3, 2))
        embeddings_1 = self.lin(embeddings.view(embeddings.shape[0], self.feature_map_size))
        embeddings_2 = minitorch.dropout(embeddings_1, 0.25)
        return embeddings_2.sigmoid().view(embeddings_2.shape[0])
